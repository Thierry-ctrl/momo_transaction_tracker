...
## Project Description
A full-stack application that processes Mobile Money (MoMo) transaction messages from JSON data, stores them in a database, and exposes them through a RESTful API. 

## Team Member
- Thierry Rugira

## Project Resources
- **Source Data:** `dsa/momo_messages.json` (Raw JSON transaction records)
- **Parsed Output:** `dsa/transactions.json` (generated by the parser)
- **Database:** `momo_tracker.db` (SQLite, generated by setup script)

---

## Assignment 1: JSON Parser & RESTful API

### Features Implemented
1. **JSON Source Parser:** Reads `momo_messages.json` and processes it into a structured list saved as `transactions.json`.
